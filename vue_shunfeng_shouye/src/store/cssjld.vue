  ...
  <section class="myAddress">
    <section>
      <section class="cont" @click="choseAdd()">
        <section>
          <span
            >所在地区：{{ Province ? Province : "" }}
            {{ City && City !== "请选择" ? City : "" }}
            {{ District && District !== "请选择" ? District : "" }}</span
          >
        </section>
      ...
      <section class="address">
        ...
        <ul>
          ...
          <li
            class="addList"
            v-for="(v, k) in info"
            @click="getProvinceId(v.index, v.AREA_NAME, k)"
            :key="k"
            v-show="showProvince"
          ...
          <li
            ...
            :key="k"
            @click="getCityId(v.index, v.AREA_NAME, k)"
            v-show="showCity"
            :class="v.selected ? 'active' : ''"
          >
            {{ v.AREA_NAME }}
          ...
          <li
            ...
            :key="k"
            @click="getDistrictId(v.index, v.AREA_NAME, k)"
            v-show="showDistrict"
            :class="v.selected ? 'active' : ''"
          >
            {{ v.AREA_NAME }}
...
export default {
  ...
  methods: {
    ...
    getAreaData() {
      console.log("获取省市区数据");
      this.$root._axios("get", "url", {}).then(res => {
...
.cityName {
  ...
  margin: 0.1rem 0.1rem 0.3rem 0.6rem;
  font-size: 0.29rem;
}
</style>
